...
- **可配置排队**：`config.yaml` 的 `concurrency.max_queue` 控制「除正在执行的 1 个外，还能排队多少个」。
  - `max_queue: 0` → 严格串行：**同一时刻仅允许 1 个请求被接纳**，第 2 个并发请求直接 503 拒绝（最严，适合单账号防风控）；首个任务可正常执行。
  - `max_queue: N`（默认 3）→ 允许 N 个请求挂起等待，表现为「稍等片刻」。
- **容量判定**：实例同时接纳任务数 = `1（执行中） + max_queue（等待中）`。判定条件为 `执行中任务数 + 队列等待数 >= 1 + max_queue` 时返回 `503`（**不是** `len(queue) >= max_queue`，该写法在 `max_queue=0` 时会连首个任务也拒绝，属错误实现，已修正）。
- **排队满拒绝**：实例已达容量上限时，新请求返回 `503`，响应体 `{"error":"系统繁忙，请稍后再试（服务器并发已达上限）"}`，**不卡连接**。
...
- 单实例串行执行（FIFO 队列 + `serve()` 协程），后端以 `503 系统繁忙` 拒绝超量并发，避免任务叠加与页面交叉。容量判定基于「执行中 + 等待中」对比 `1 + max_queue`，`max_queue=0` 时仅拒第 2 个并发、首个可正常执行。
