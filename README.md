...
# 单实例串行与排队（核心能力）
concurrency:
  # 每个 WS 实例（浏览器标签页）允许"排队等待"的最大请求数（不含正在执行的 1 个）。
  # 实例同时可接纳任务数 = 1（执行中）+ max_queue（等待中）。
  #   0 = 严格串行：仅允许 1 个执行中，第 2 个并发请求立即 503 拒绝；
  #   3 = 1 个执行 + 3 个排队，第 5 个被拒。
  # 注意：max_queue=0 时也能正常执行首个任务（容量判定基于"执行中+等待中"，
  # 而非单纯队列长度——早期版本曾误用 len(queue)>=0 导致连首个也被拒，已修复）。
  max_queue: 3
...
- **可配置排队**：`config.yaml` 的 `concurrency.max_queue` 控制「除正在执行的 1 个外，还能排队多少个」。默认 `3`（即 1 执行 + 3 等待）。
  - `max_queue: 0` → 严格串行：**同一时刻只允许 1 个请求被接纳**，第 2 个并发请求直接返回 503 拒绝（最严格，适合单账号防风控）。首个任务可正常执行，仅拒"第 2 个及以后"。
  - `max_queue: N` → 允许 N 个请求挂起等待，对客户端表现为「稍等片刻」。
- **容量判定**：实例同时接纳任务数 = `1（执行中） + max_queue（等待中）`。判定条件为 `执行中任务数 + 队列等待数 >= 1 + max_queue` 时返回 `503`，**不是** `len(queue) >= max_queue`（后者在 `max_queue=0` 时会连首个任务也拒绝，属错误实现，已修正）。
- **排队满拒绝**：当某实例已达容量上限，新请求入队时返回 `503`，响应体 `{"error":"系统繁忙，请稍后再试（服务器并发已达上限）"}`，**不会卡住连接**。
...
| 设 `max_queue: 0` 后**任何请求都 503** | 旧版容量判定误用 `len(queue) >= 0` 恒成立，连首个任务也拒（已修复） | 升级到修复版本；正常情况下 `max_queue: 0` 仅拒第 2 个并发、首个可正常执行 |
